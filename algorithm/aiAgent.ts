...
const selectFirstCard = (cards: CardType[]): CardType => {
  const availableCards = cards.filter((card) => !card.finished && !card.selected);

  // 開かれているカードで重複しているものがある場合は、そのカードを選択する
  const openedCards = availableCards.filter((card) => card.opened);
  ...
  const notOpenedCards = availableCards.filter((card) => !card.opened);
...
const selectSecondCard = (cards: CardType[]): CardType => {
  ...
  const firstCard = cards.filter((card) => card.selected);
  const openedCards = availableCards.filter((card) => card.opened);

  // 開かれたことがあるカードで1枚目と同じカードがあれば、そのカードを選択する
  const sameCards = openedCards.filter(
    (card) => card.image_id === firstCard[0].image_id,
  );
